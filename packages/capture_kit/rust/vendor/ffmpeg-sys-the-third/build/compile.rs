...
pub fn build(libraries: &[Library], out_dir: &Path) -> io::Result<PathBuf> {
    ...
    // capture_kit patch: `--disable-autodetect` above also turns off
    // VideoToolbox (an autodetected-only feature upstream never gates behind
    // its own Cargo feature/`configure.enable(...)` call, unlike libdrm/nvenc
    // just below) - re-enable it explicitly on Apple targets so
    // `h264_videotoolbox`/`AV_HWDEVICE_TYPE_VIDEOTOOLBOX` actually exist in
    // the built library, instead of `av_hwdevice_ctx_create` failing with a
    // misleading ENOMEM ("Cannot allocate memory") because
    // `av_hwdevice_ctx_alloc`'s internal `get_hw_type` lookup silently finds
    // nothing for a device type FFmpeg's own configure decided not to build.
    // No effect on non-Apple targets (the framework doesn't exist there
    // regardless).
    //
    // Deliberately NOT `--enable-audiotoolbox`: this crate only needs
    // VideoToolbox (video). Enabling audiotoolbox too compiles in
    // audiotoolboxenc.o/audiotoolboxdec.o/audiotoolbox.o, which reference
    // AudioToolbox/AudioQueue framework symbols this crate then never links
    // against anywhere - `-force_load`ing the resulting static lib into an
    // app that also never links AudioToolbox fails at final link time with
    // undefined symbols (_AudioConverterNew, _AudioQueueStart, etc.), which
    // is exactly what happened when this was enabled.
