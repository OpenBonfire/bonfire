...
The `output.264` you created should be in the same directory as `play-from-disk-h26x`. In the jsfiddle the top textarea
is your browser, copy that and:

#### Linux/macOS

1. Run
   `echo $BROWSER_SDP | cargo run --example play-from-disk-h26x -- -v examples/test-data/output.264 -a examples/test-data/output.ogg`

2. Run
   `echo $BROWSER_SDP | cargo run --example play-from-disk-h26x -- -v examples/test-data/output.265 -a examples/test-data/output.ogg --hevc`

#### Windows

1. Paste the SessionDescription into a file.
2. Run
   `cargo run --example play-from-disk-h26x -v examples/test-data/output.264 -a examples/test-data/output.ogg < my_file`
3. Run
   `cargo run --example play-from-disk-h26x -v examples/test-data/output.265 -a examples/test-data/output.ogg --hevc < my_file`
