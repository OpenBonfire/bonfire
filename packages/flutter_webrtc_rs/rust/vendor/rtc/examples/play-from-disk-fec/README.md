...
The repository already ships a usable file at `examples/test-data/output_vp8.ivf`, which the commands below use.
...
```shell
cargo run --example play-from-disk-fec -- -v examples/test-data/output_vp8.ivf
```
...
```shell
cargo run --example play-from-disk-fec -- -v examples/test-data/output_vp8.ivf -i answer.txt
```
