...
void checkMessage(Message message) {
  expect(message.id, equals(Snowflake(BigInt.from(334385199974967042))));
  expect(message.author.id, equals(Snowflake(BigInt.from(53908099506183680))));
...
void checkCrosspostedMessage(Message message) {
  ...
  expect(message.channelMentions.single.guildId,
      equals(Snowflake(BigInt.from(278325129692446720))));
  expect(message.channelMentions.single.id,
      equals(Snowflake(BigInt.from(278325129692446722))));
  ...
  expect(message.activity, isNull);
  expect(message.applicationId, isNull);
  expect(message.reference?.channelId,
      equals(Snowflake(BigInt.from(278325129692446722))));
  expect(message.reference?.guildId,
      equals(Snowflake(BigInt.from(278325129692446720))));
  expect(message.reference?.messageId,
      equals(Snowflake(BigInt.from(306588351130107906))));
...
void checkMessageInteractionMetadata(MessageInteractionMetadata metadata) {
  expect(metadata.id, equals(Snowflake(BigInt.from(1234567891234567800))));
  expect(metadata.type, equals(InteractionType.applicationCommand));
  expect(metadata.userId, equals(Snowflake(BigInt.from(1234567891234567801))));
  expect(
      metadata.authorizingIntegrationOwners,
      equals({
        ApplicationIntegrationType.guildInstall:
            Snowflake(BigInt.from(1234567891234567802)),
        ApplicationIntegrationType.userInstall:
            Snowflake(BigInt.from(1234567891234567803)),
      }));
  expect(metadata.originalResponseMessageId,
      Snowflake(BigInt.from(1234567891234567804)));
  expect(metadata.interactedMessageId,
      Snowflake(BigInt.from(1234567891234567805)));
  ...
  expect(metadata2.id, equals(Snowflake(BigInt.from(1234567891234567806))));
  expect(metadata2.type, equals(InteractionType.applicationCommand));
  expect(metadata2.userId, equals(Snowflake(BigInt.from(1234567891234567807))));
  expect(
      metadata2.authorizingIntegrationOwners,
      equals({
        ApplicationIntegrationType.guildInstall:
            Snowflake(BigInt.from(1234567891234567808)),
        ApplicationIntegrationType.userInstall:
            Snowflake(BigInt.from(1234567891234567809)),
      }));
  ...
  testManager<Message, MessageManager>(
    ...
    additionalEndpointTests: [
      ...
      EndpointTest<MessageManager, Message, Map<String, Object?>>(
        ...
        execute: (manager) => manager.crosspost(Snowflake(BigInt.from(1))),
      ...
      EndpointTest<MessageManager, void, void>(
        ...
        execute: (manager) => manager.pin(Snowflake(BigInt.from(1))),
        check: (_) {},
      ),
      EndpointTest<MessageManager, void, void>(
        name: 'unpin',
        ...
        source: null,
        urlMatcher: '/channels/0/pins/1',
        execute: (manager) => manager.unpin(Snowflake(BigInt.from(1))),
        check: (_) {},
      ),
    ],
  );
